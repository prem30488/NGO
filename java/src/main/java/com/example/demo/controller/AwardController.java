...
public class AwardController {
	...
	@GetMapping("/api/user/awards")
    public Page<Award> getAwards(Pageable pageable) {
        return awardRepository.findAll(pageable);
    }


    @PostMapping("/api/user/awards")
    public Award createAward(@Valid @RequestBody Award award) {
        return awardRepository.save(award);
    }

    @PutMapping("/api/user/awards/{awardId}")
    public Award updateAward(@PathVariable Long awardId,
                                   @Valid @RequestBody Award awardRequest) {
    ...
    @DeleteMapping("/api/user/awards/{awardId}")
    ...
    @GetMapping("/api/user/awards/{awardId}")
	public Award fetchUserById(@PathVariable Long awardId) {
		return awardRepository.findById(awardId)
				.orElseThrow(() -> new ResourceNotFoundException("Award not found with id " + awardId));
	}
}
